...
class TaskManagementService {
    ...
    try {
      ...
      if (availableTasks.length > 0) {
        availableTasks.forEach((task, index) => {
          Logger.info(`   ${index + 1}. "${task.title}" (ID: ${task.id}, 우선순위: ${task.priority}, 상태: ${task.status}, 할당로봇: ${task.robot_id || 'none'})`);
        });
      }
  ...
  async assignTaskToRobot(robot, task) {
    try {
      Logger.info(`🎯 로봇 ${robot.name}(ID: ${robot.id})에 태스크 "${task.title}"(ID: ${task.id}) 할당 시작`);
      ...
      Logger.info(`🎉 태스크 할당 완료: ${robot.name} -> ${task.title}`);

    } catch (error) {
      Logger.error(`❌ 태스크 할당 실패 (로봇: ${robot.name}, 태스크: ${task.title}):`, { error: error.message, stack: error.stack });
    }
  }
  ...
  async manageRobotTask(robot) {
    try {
      ...
      Logger.debug(`🔍 로봇 ${robot.name}: 태스크 "${currentTask.title}" 진행 상황 확인 (order_status: ${robot.order_status})`);
  ...
  async completeTask(robot, task) {
    try {
      Logger.info(`🎉 로봇 ${robot.name}: 태스크 "${task.title}" 완료 처리 시작`);
      ...
      Logger.info(`✅ 태스크 완료: ${robot.name} -> ${task.title}`);
  ...
  async sendMoveCommand(robot, waypoint) {
    try {
      ...
      const commandData = {
        ...
        type: "navigate",
        params: {
          goto_node_id: String(waypoint.stationId || waypoint.id || waypoint.index || 1)
        }
      };
